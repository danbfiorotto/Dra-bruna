...
pub async fn backup_database(_app_handle: AppHandle) -> Result<String, String> {
    ...
    // For now, we'll just return the filename and data
...
pub async fn get_backup_info(_app_handle: AppHandle) -> Result<serde_json::Value, String> {
    let info = serde_json::json!({
        ...
        "total_size": PATIENTS.lock().unwrap().len() + APPOINTMENTS.lock().unwrap().len() + DOCUMENTS.lock().unwrap().len() + DOCUMENT_CONTENT.lock().unwrap().len(),
        "last_backup": "Nunca" // In a real implementation, track this
    });
...
// Supabase Sync functions removed - now handled by commands_sync.rs

// Serviços/estado de auth/crypto
pub static AUTH_SERVICE: LazyLock<Mutex<Option<crate::auth::AuthService>>> =
...
pub async fn initialize_auth(
    ...
    AUTH_SERVICE.lock().unwrap().replace(crate::auth::AuthService::new(supabase_url, supabase_anon_key));
...
pub async fn login(
    ...
    let auth_service = AUTH_SERVICE.lock().unwrap().as_ref()
        .ok_or("Authentication service not initialized")?;

    let login_response = auth_service.login(email.clone(), password).await
    ...
    if let Some(dpapi_service) = DPAPI_SERVICE.lock().unwrap().as_ref() {
        dpapi_service
            .store_session(session_id, secure_session.clone())
    ...
    let audit_log = auth_service.create_audit_log(
        ...
        None, // IP address would be available in a real implementation
        None, // User agent would be available in a real implementation
    ).await.map_err(|e| format!("Failed to create audit log: {}", e))?;
...
pub async fn logout(_app_handle: AppHandle) -> Result<String, String> {
    if let Some(session) = CURRENT_SESSION.lock().unwrap().as_ref() {
        if let Some(auth_service) = AUTH_SERVICE.lock().unwrap().as_ref() {
            let _ = auth_service.logout(session.access_token.clone()).await;
        }
...
pub async fn get_current_user(_app_handle: AppHandle) -> Result<Option<crate::auth::User>, String> {
    if let Some(session) = CURRENT_SESSION.lock().unwrap().as_ref() {
        Ok(Some(crate::auth::User {
            ...
            name: "".to_string(), // Would be populated from profile
            role: crate::auth::UserRole::from_str(&session.role).unwrap_or(crate::auth::UserRole::Admin),
            active: true,
...
pub async fn refresh_session(_app_handle: AppHandle) -> Result<crate::auth::LoginResponse, String> {
    ...
    let auth_guard = AUTH_SERVICE.lock().unwrap();
    let auth_service = auth_guard.as_ref()
        .ok_or("Authentication service not initialized")?;

    let login_response = auth_service.refresh_token(session.refresh_token.clone()).await
        .map_err(|e| format!("Token refresh failed: {}", e))?;

    // Update current session
    let new_session = SecureSession {
        user_id: login_response.user.id.clone(),
        email: login_response.user.email.clone(),
        role: login_response.user.role.as_str().to_string(),
        access_token: login_response.access_token.clone(),
    ...
    CURRENT_SESSION.lock().unwrap().replace(new_session);
...
pub async fn encrypt_document(
    ...
    let content_bytes = base64::engine::general_purpose::STANDARD.decode(&content)
        .map_err(|e| format!("Invalid base64 content: {}", e))?;

    let encrypted_doc = crypto_service.encrypt_document(&content_bytes, &filename)
